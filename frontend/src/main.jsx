...
function joinMultiBlankAnswer(parts = []) {
  // Input ichida 'is swimming' kabi javoblarda probel o'chib ketmasligi uchun
  // yozish paytida bo'sh joylarni trim qilmaymiz. Tekshirish joyida baribir trim/normalize qilinadi.
...
function AdminPanel({ onLogout }) {
  ...
  const [message, setMessage] = useState('');
  const [form, setForm] = useState({ role: 'student', username: '', password: '', fullName: '', subject: 'english', isActive: true, expiresAt: todayPlus(90), paymentStatus: 'paid', paymentAmount: '', paymentNote: '', allowUnlimitedTopics: false, topicAccessMode: 'daily', centerId: 'center_main' });
  const [editingUser, setEditingUser] = useState(null);
  const [editForm, setEditForm] = useState({ username: '', password: '', fullName: '', subject: 'english', isActive: true, expiresAt: '', paymentStatus: 'paid', paymentAmount: '', paymentNote: '', topicAccessMode: 'daily', allowUnlimitedTopics: false });
  ...
  async function createUser(e) {
    ...
    try {
      ...
      setForm({ role: roleAfterCreate, username: '', password: '', fullName: '', subject: defaultSubjectForRole(roleAfterCreate), isActive: true, expiresAt: todayPlus(90), paymentStatus: 'paid', paymentAmount: '', paymentNote: '', allowUnlimitedTopics: false, topicAccessMode: 'daily', centerId: form.centerId || selectedCenterId || 'center_main' });
      await reloadAll();
    } catch (err) {
      setMessage(err.message);
    }
  ...
  function openEditUser(user) {
    ...
    setEditForm({
      ...
      topicAccessMode: user.topicAccessMode || (user.allowUnlimitedTopics ? 'unlimited' : 'daily'),
      allowUnlimitedTopics: !!user.allowUnlimitedTopics
    });
  }
  ...
  async function saveEditUser(e) {
    ...
    const patch = { ...editForm, isActive: editForm.paymentStatus === 'paid' && !!editForm.isActive };
    if (!patch.password) delete patch.password;
    try {
      const data = await api(`/api/admin/users/${editingUser.id}`, { method: 'PATCH', body: JSON.stringify(patch) });
      setMessage(data.message || 'Foydalanuvchi yangilandi');
      setEditingUser(null);
      await reloadAll();
    } catch (err) {
      setMessage(err.message);
        ...
        <section className="adminContentPanel">
          ...
          {editingUser && (
            <div className="modalBackdrop" onClick={() => setEditingUser(null)}>
              <form className="card formGrid editUserModal" onSubmit={saveEditUser} onClick={e => e.stopPropagation()}>
                <div className="modalHeader"><h2>Foydalanuvchini tahrirlash</h2><button type="button" className="ghost small" onClick={() => setEditingUser(null)}>×</button></div>
                <label>Login</label>
                <input value={editForm.username} onChange={e => setEditForm({ ...editForm, username: e.target.value })} />
                <label>Yangi parol</label>
                <input value={editForm.password} onChange={e => setEditForm({ ...editForm, password: e.target.value })} placeholder="Bo‘sh qoldirilsa o‘zgarmaydi" />
                <label>Ism familya</label>
                <input value={editForm.fullName} onChange={e => setEditForm({ ...editForm, fullName: e.target.value })} />
                <label>Fan</label>
                <select value={editForm.subject} onChange={e => setEditForm({ ...editForm, subject: e.target.value })}>
                  {editingUser.role === 'admin' && canAll && <option value="all">Barcha fanlar</option>}
                  {subjectOptions.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
                </select>
                <label>Holati</label>
                <select value={editForm.isActive ? 'active' : 'non-active'} disabled={editForm.paymentStatus === 'unpaid'} onChange={e => setEditForm({ ...editForm, isActive: e.target.value === 'active' })}>
                  <option value="active">Active</option>
                  <option value="non-active">Non-active</option>
                </select>
                ...
                {editingUser.role === 'student' && (
                  <>
                    <label>To‘lov holati</label>
                    <select value={editForm.paymentStatus || 'paid'} onChange={e => setEditForm({ ...editForm, paymentStatus: e.target.value, isActive: e.target.value === 'paid' })}>
                      <option value="paid">To‘langan</option>
                      <option value="unpaid">To‘lanmagan</option>
                    </select>
                    <label>To‘lov izohi</label>
                    ...
                    <label>Hamma mavzular</label>
                    <label className="inlineCheck adminUnlimitedCheck">
                      <input type="checkbox" checked={(editForm.topicAccessMode || (editForm.allowUnlimitedTopics ? 'unlimited' : 'daily')) === 'unlimited'} onChange={e => setEditForm({ ...editForm, topicAccessMode: e.target.checked ? 'unlimited' : 'daily', allowUnlimitedTopics: e.target.checked })} />
                      <span>Cheklovsiz: hamma mavzular ochiq bo‘lsin</span>
                    </label>
                  </>
                )}
                  ...
                  {filteredUsers.map(user => (
                    <div className={`tr ${selectedAccountIds.includes(user.id) ? 'checkedRow' : ''} ${selectedUserId === user.id ? 'selectedRow' : ''}`} key={user.id}>
                      ...
                      <span className="rowActions accountRowActions">
                        ...
                        <button type="button" className="small" onClick={() => openEditUser(user)}>Tahrirlash</button>
